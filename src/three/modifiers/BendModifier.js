...
import { Vector3, Matrix3, Matrix4 } from "three-full";

const BendModifier = function() {};

BendModifier.prototype = {
  constructor: BendModifier,
  ...
  modify: function(geometry) {
    var thirdAxis = new Vector3();
    thirdAxis.crossVectors(this.direction, this.axis);
    ...
    var InverseP = new Matrix4().getInverse(P);
    var newVertices = [];
    var oldVertices = [];
    var anglesBetweenOldandNewVertices = [];

    var meshGeometryBoundingBoxMaxx = 0;
    var meshGeometryBoundingBoxMinx = 0;
    var meshGeometryBoundingBoxMaxy = 0;
    var meshGeometryBoundingBoxMiny = 0;

    for (var i = 0; i < geometry.vertices.length; i++) {
      newVertices[i] = new Vector3();
      newVertices[i].copy(geometry.vertices[i]).applyMatrix4(InverseP);
    ...
    var meshWidthold =
      meshGeometryBoundingBoxMaxx - meshGeometryBoundingBoxMinx;
    ...
    var oldMiddlex =
      (meshGeometryBoundingBoxMaxx + meshGeometryBoundingBoxMinx) / 2;
    var oldMiddley =
      (meshGeometryBoundingBoxMaxy + meshGeometryBoundingBoxMiny) / 2;

    for (i = 0; i < geometry.vertices.length; i++) {
      oldVertices[i] = new Vector3();
      oldVertices[i].copy(newVertices[i]);
      newVertices[i].x =
        this._sign(newVertices[i].x - oldMiddlex) *
        1 /
        ParamB *
        this._sinhInverse((newVertices[i].x - oldMiddlex) * ParamB);
    }
    ...
    rightEdgePos.y =
      1 / NewParamB * this._cosh(NewParamB * rightEdgePos.x) -
      1 / NewParamB -
      meshDepth / 2;

    var bendCenter = new Vector3(
      0,
      rightEdgePos.y + rightEdgePos.x / Math.tan(this.angle),
      0
    );

    for (i = 0; i < geometry.vertices.length; i++) {
      var x0 =
        this._sign(oldVertices[i].x - oldMiddlex) *
        1 /
        ParamB *
        this._sinhInverse((oldVertices[i].x - oldMiddlex) * ParamB);
      var y0 = 1 / NewParamB * this._cosh(NewParamB * x0) - 1 / NewParamB;

      var k = new Vector3(
        bendCenter.x - x0,
        bendCenter.y - (y0 - meshDepth / 2),
        bendCenter.z
      ).normalize();

      var Q = new Vector3();
      Q.addVectors(
        new Vector3(x0, y0 - meshDepth / 2, oldVertices[i].z),
        k.multiplyScalar(oldVertices[i].y + meshDepth / 2)
      );
      newVertices[i].x = Q.x;
      newVertices[i].y = Q.y;
    }
    ...
    for (i = 0; i < geometry.vertices.length; i++) {
      ...
      var p = new Vector3();
      p.subVectors(oldVertices[i], O);
      var q = new Vector3();
      q.subVectors(newVertices[i], O);

      anglesBetweenOldandNewVertices[i] =
        Math.acos(1 / this._cosh(ParamB * newVertices[i].x)) *
        this._sign(newVertices[i].x);
    ...
    for (var f = 0, fl = geometry.faces.length; f < fl; f++) {
      ...
      for (var v = 0, vl = face.vertexNormals.length; v < vl; v++) {
        ...
        rotateMatrix.set(
          ...
          (1 - Math.cos(angle)) * y * x + Math.sin(angle) * z,
          Math.cos(angle) + (1 - Math.cos(angle)) * y * y,
          (1 - Math.cos(angle)) * y * z - Math.sin(angle) * x,
          (1 - Math.cos(angle)) * z * x - Math.sin(angle) * y,
          (1 - Math.cos(angle)) * z * y + Math.sin(angle) * x,
