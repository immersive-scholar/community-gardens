...
class Highres {
  ...
  keyHandler(e) {
    ...
    if (e.key.match(/^\d+$/)) {
      ...
      // Get factor
      const factor = Math.min(parseInt(e.key, 10), this.state.maxFactor);
  ...
  createSMAAPass(done) {
    ...
    const areaTexture = this.smaaPass.materialWeights.uniforms.tArea.value;
    ...
    // Reload.
    if (this.smaaPass.materialWeights.uniforms.tArea.image) {
      areaTexture.image.src = this.smaaPass.materialWeights.uniforms.tArea.image.src;
    }
    if (this.smaaPass.materialWeights.uniforms.tSearch.image) {
      searchTexture.image.src = this.smaaPass.materialWeights.uniforms.tSearch.image.src;
    }
  }
  ...
  renderNormal(array, size) {
    // METHOD 1 - Normal render target + js renderer (fastest, no Anti-Aliasing)
    // const anisotropy = this.renderer.capabilities.getMaxAnisotropy()
    // this.rt = new WebGLRenderTarget(size.width, size.height);
